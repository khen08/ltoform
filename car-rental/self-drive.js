document.addEventListener('DOMContentLoaded', function () {
  displayRandomCars('Hyper', 2);
  displayRandomCars('Super', 2);
  displayRandomCars('Sports', 1);
  displayRandomCars('IconicJDM', 1);
});

function displayRandomCars(category, count) {
  fetch('cars.json')
    .then((response) => response.json())
    .then((carsJson) => {
      const selectedCars = getRandomElements(carsJson[category], count);
